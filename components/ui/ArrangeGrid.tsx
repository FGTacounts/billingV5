...
function DropSlot() {
  return (
    <div className="lg:col-span-2 min-h-[336px] rounded-card border-2 border-dashed border-accent/60 bg-accent/8 grid place-items-center">
      <span className="text-subhead font-semibold text-accent">{t("ui.dropHere")}</span>
    </div>
  );
}
...
// The category name is data — it is what a catalog entry is tagged with — so
// the heading printed above each group is looked up rather than shown raw.
...
function WidgetPreview({ children }: { children: ReactNode }) {
  return (
    <div className="w-[188px] h-[104px] rounded-inner overflow-hidden bg-canvas border border-hairline relative shrink-0">
      <div
        aria-hidden
        className="absolute top-0 start-0 origin-top-left pointer-events-none select-none"
    ...
    <>
            ...
            {editing && (
              <>
                ...
                <button
                  ...
                  className="absolute -top-2 -start-2 z-30 w-6 h-6 rounded-full bg-[--status-danger] text-white grid place-items-center shadow-floating"
                  aria-label={t("ui.removeNamedWidget", { label: labels[key] })}
                  title={t("ui.removeNamedWidget", { label: labels[key] })}
                >
                  <Minus size={14} />
                </button>
                {/* Drag handle */}
                <div
                  ...
                  className="absolute -top-2 -end-2 z-30 w-6 h-6 rounded-full bg-surface border border-hairline text-secondary grid place-items-center shadow-floating cursor-grab active:cursor-grabbing"
                  aria-label={t("ui.dragNamedWidget", { label: labels[key] })}
      ...
      {editing && (
        <motion.div
          ...
          transition={springLayout}
          className="fixed start-0 end-0 bottom-0 z-40 md:pl-[var(--sidebar-w,0px)]"
        >
          <div className="mx-3 mb-3 md:mx-6 md:mb-6 glass rounded-sheet shadow-floating p-4">
            <div className="flex items-center justify-between mb-3 gap-3">
              <span className="text-headline font-semibold">{t("ui.widgetGallery")}</span>
              <button
            ...
            <div className="max-h-[38vh] overflow-y-auto flex flex-col gap-3">
                      ...
                      <div className="flex gap-3 flex-wrap">
                        {items.map((w) => (
                          ...
                          >
                            ...
                            <span className="text-caption text-secondary text-left">{t("ui.fromSource", { source: w.from })}</span>
                          </button>
                        ))}
                      </div>
                    </div>
              ...
              {hiddenKeys.length === 0 && !onAddFromCatalog && (
                <p className="text-caption text-secondary">{t("ui.everyWidgetOnPage")}</p>
              )}
            </div>
          </div>
        </motion.div>
