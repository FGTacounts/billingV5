...
export default function ImportOrderLinesButton({
  ...
  async function handleFile(file: File) {
    ...
    try {
      ...
      const wanted = rows
        ...
        .filter((r) => r.sku !== "");
      if (wanted.length === 0) {
        toast.error(t("orders.importNoSkuColumn"));
        return;
      }
      ...
      if (matched.length === 0) {
        toast.error(t("orders.importNoneMatched", { count: wanted.length }));
        return;
      }
      onImported(matched);
      if (missing.length > 0) {
        toast.error(
          (matched.length === 1
            ? t("orders.importAddedOne", { n: matched.length })
            : t("orders.importAddedMany", { n: matched.length })) +
            (missing.length === 1
              ? t("orders.importUnmatchedOne", {
                  n: missing.length,
                  list: missing.slice(0, 10).join(", "),
                })
              : t("orders.importUnmatchedMany", {
                  n: missing.length,
                  list: missing.slice(0, 10).join(", "),
                })) +
            (missing.length > 10 ? t("orders.importMoreUnmatched", { n: missing.length - 10 }) : "")
        );
      }
    } catch (e) {
      toast.error(friendlyError(e, t("orders.importFailed")));
    } finally {
      setBusy(false);
    }
  }
  ...
  return (
    <div className="flex items-center gap-3">
      ...
      >
        <Upload size={14} /> {busy ? t("orders.importing") : t("orders.importItems")}
      </button>
      <button
        type="button"
        onClick={() =>
      ...
      >
        <Download size={14} /> {t("orders.sample")}
      </button>
    </div>
  );
}
