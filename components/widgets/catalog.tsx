...
function TopCustomersWidget() {
  ...
  return (
    <Card className="p-4">
      <div className="text-title font-bold mb-3">{t("dashboard.topCustomers")}</div>
      {rows === null ? (
        <SkeletonRows />
      ) : rows.length === 0 ? (
        <div className="text-center text-subhead text-secondary py-8">{t("dashboard.noSalesThisMonthYet")}</div>
      ) : (
...
function SaleTrendWidget() {
  ...
  return (
    <Card className="p-4">
      <div className="text-title font-bold mb-3">{t("dashboard.saleTrend30Days")}</div>
      {trend === null ? (
        <SkeletonRows n={3} />
      ) : (
...
function DetailedSalesWidget() {
  ...
  return (
    <Card className="p-4 h-full flex flex-col">
      <div className="text-title font-bold mb-3 shrink-0">{t("dashboard.detailedSales")}</div>
      <div className="flex-1 min-h-0 overflow-y-auto">
        <table className="w-full text-caption">
          <thead>
            <tr className="text-secondary uppercase text-left sticky top-0 bg-surface">
              <th className="py-1.5 font-medium">{t("sales.month")}</th>
              <th className="py-1.5 font-medium text-right">{t("sales.sale")}</th>
              <th className="py-1.5 font-medium text-right">{t("dashboard.tgtPctShort")}</th>
              <th className="py-1.5 font-medium text-right">{t("dashboard.pastYrShort")}</th>
            </tr>
          </thead>
          <tbody>
            {(rows ?? []).map((m) => (
              <tr key={m.label} className="border-t border-hairline">
                ...
                <td className="py-1.5 text-right tabular-nums text-secondary">
                  {m.target > 0 ? `${Math.round((m.sale / m.target) * 100)}%` : t("common.notSet")}
                </td>
...
function GoalLeaderboardWidget() {
  ...
  return (
    <Card className="p-4 h-full flex flex-col">
      <div className="text-title font-bold mb-3 shrink-0">{t("sales.goal")}</div>
...
function PaymentsSummaryWidget() {
  ...
  const rows = [
    { label: t("dashboard.collected"), value: s?.received ?? 0, cls: "text-accent" },
    { label: t("dashboard.remaining"), value: s?.remaining ?? 0, cls: "text-secondary" },
    { label: t("dashboard.overdue"), value: s?.overdue ?? 0, cls: "text-[--status-danger]" },
  ];
  return (
    <Card className="p-4 h-full flex flex-col">
...
function SalesByCategoryWidget() {
  ...
  return (
    <Card className="p-4 h-full flex flex-col">
      <div className="text-title font-bold mb-3 shrink-0">{t("dashboard.salesByCategory")}</div>
      <div className="flex-1 min-h-0 overflow-y-auto flex flex-col gap-1.5">
        {(rows ?? []).slice(0, 12).map((r) => (
          <div key={r.label} className="relative rounded-inner overflow-hidden px-3 py-2">
            <div
              className="absolute inset-y-0 start-0 rounded-inner"
...
export const WIDGET_CATALOG: CatalogWidget[] = [
  { key: "cat:saleTrend", label: t("dashboard.saleTrend"), category: "Sales", from: t("nav.sales"), render: () => <SaleTrendWidget /> },
  { key: "cat:yearComparison", label: t("dashboard.yearComparison"), category: "Sales", from: t("nav.sales"), render: () => <YearComparisonWidget /> },
  { key: "cat:detailedSales", label: t("dashboard.detailedSalesReport"), category: "Sales", from: t("nav.sales"), render: () => <DetailedSalesWidget /> },
  { key: "cat:goal", label: t("sales.widgetGoal"), category: "Sales", from: t("nav.sales"), render: () => <GoalLeaderboardWidget /> },
  { key: "cat:salesByCategory", label: t("dashboard.salesByCategory"), category: "Sales", from: t("nav.dashboard"), render: () => <SalesByCategoryWidget /> },
  { key: "cat:topCustomers", label: t("dashboard.topCustomers"), category: "Customers", from: t("nav.customers"), render: () => <TopCustomersWidget /> },
  { key: "cat:paymentsSummary", label: t("dashboard.paymentsSummary"), category: "Payments", from: t("nav.payments"), render: () => <PaymentsSummaryWidget /> },
  { key: "cat:expense", label: t("dashboard.expenseBreakdown"), category: "Expense", from: t("nav.expense"), render: () => <ExpenseBreakdownWidget /> },
];
