...
export interface UserPreferences {
  ...
  // Reports page widget board (the iOS Reports tab's arrangeable widgets) —
  // same shape as dashboardLayout / dashboardHidden / dashboardSizes.
  reportsLayout?: string[];
  reportsHidden?: string[];
  reportsSizes?: Record<string, "full" | "half" | "quarter">;
}
...
export interface Product {
  ...
  // Products with the same id are one physical stock sold under several
  // SKUs; the database keeps their stock_on_hand identical. null = own
  // stock. Optional because the column doesn't exist until
  // scratchpad/RUN-ME-18-shared-stock.sql is run.
...
export interface GrvReturn {
  ...
  approved_by: string | null;
  created_at: string;
  // The three below arrive with scratchpad/RUN-ME-25. `amount` is the credit
  // given, VAT included; null means "value it from the lines", which is every
  // return logged before 2026-09-18. `payment_id` is set when the return was
  // claimed while collecting a payment.
  amount?: number | null;
