  ...
  for (const o of rows) {
    ...
    const entry: DuplicateOrder = {
      ...
      customerName: o.customer_id
        ? names.get(o.customer_id) ?? t("orders.unknownCustomer")
        : o.new_customer_note ?? t("orders.unnamedCustomer"),
      salesman: o.salesman_id ? salesmen.get(o.salesman_id) ?? t("common.notSet") : t("common.notSet"),
...
export async function POST(req: NextRequest) {
  ...
  if (!Array.isArray(orderIds) || orderIds.length === 0) {
    return NextResponse.json({ error: t("orders.nothingSelectedToRemove") }, { status: 400 });
  }
  ...
  if (deletable.length === 0) {
    return NextResponse.json(
      { error: t("orders.duplicatesAlreadyMovedOn") },
      { status: 409 }
    );
  }
