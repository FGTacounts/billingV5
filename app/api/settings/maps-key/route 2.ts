...
export async function POST(req: NextRequest) {
  ...
  if (typeof apiKey !== "string" || !apiKey.trim()) {
    return NextResponse.json({ error: t("settings.apiKeyRequired") }, { status: 400 });
  }
  ...
  if (!settings) {
    return NextResponse.json({ error: t("settings.noAppSettingsRow") }, { status: 500 });
  }
  ...
  if (error) {
    return NextResponse.json(
      { error: error.message || t("settings.couldntSaveMapsKey") },
      { status: 400 }
    );
  }
