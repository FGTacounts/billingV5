...
export default function Toaster() {
  ...
  return createPortal(
    <div className="fixed z-[60] start-1/2 -translate-x-1/2 bottom-[calc(env(safe-area-inset-bottom)+72px)] md:bottom-6 flex flex-col gap-2 w-[calc(100vw-32px)] max-w-[420px] pointer-events-none">
        ...
        {toasts.map((item) => {
          ...
          return (
            ...
            >
              ...
              <span className="text-subhead flex-1 min-w-0">{item.message}</span>
              <button
                onClick={() => dismiss(item.id)}
                className="shrink-0 text-secondary hover:text-primary -me-1"
