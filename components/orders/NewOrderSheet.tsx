...
export default function NewOrderSheet({
  ...
  async function save(status: "draft" | "pending") {
    ...
    try {
      ...
      if (!res.ok) throw new Error(data.error ?? t("orders.saveOrderFailed"));
      onCreated();
    } catch (e) {
      toast.error(friendlyError(e, t("orders.saveOrderFailed")));
    } finally {
      setSaving(false);
    }
  }
        ...
        footer={
          <>
            ...
            <Button tier="primary" disabled={saving || !canSave} onClick={() => save("pending")}>
              {saving ? t("orders.sending") : t("orders.sendOrder")}
            </Button>
          </>
        }
      >
        ...
        <div className="flex items-end justify-between mb-4 rounded-card bg-canvas px-4 py-3">
          ...
          <div className="text-right">
            <div className="text-caption text-secondary leading-none mb-1">{t("orders.date")}</div>
        ...
        <div className="flex items-center justify-between mb-1.5">
          <span className="text-caption text-secondary font-semibold uppercase tracking-wide">{t("orders.customer")}</span>
          {!customer && !useNewCustomer && (
            <button
              ...
              aria-label={t("orders.newCustomer")}
              title={t("orders.newCustomer")}
            >
              <Plus size={15} />
            </button>
          )}
        ...
        {!customer && !useNewCustomer && (
          <>
            ...
            <button
              ...
              onClick={() => setUseNewCustomer(true)}
            >
              <Plus size={13} /> {t("orders.newCustomer")}
              {!isManager && t("orders.requiresManagerApproval")}
            </button>
          </>
        )}
        ...
        {customer && lines.length > 0 && (
          ...
          >
            {t("orders.useLastPrices", { name: customer.name })}
          </button>
        )}

        {isManager && customer && (
          <div>
            <Label>{t("orders.orderDiscount")}</Label>
            <div className="flex items-center gap-2">
              ...
              />
              <span className="text-caption text-secondary">{t("orders.percentNoDiscountHint")}</span>
            </div>
          </div>
        )}
        ...
        {useNewCustomer && !customer && (
          <div>
            <Label>
              {isManager
                ? t("orders.newCustomerNameManager")
                : t("orders.newCustomerNameSalesman")}
            </Label>
        ...
        {holdReason && !holdWarning && (
          <div className="mt-3 p-3 rounded-card bg-canvas text-caption text-secondary">
            {t("orders.noteForManagerColon")} <span className="text-primary">{holdReason}</span>
          </div>
        )}
        ...
        <div className="flex items-center justify-between mt-4 mb-1.5 gap-3 flex-wrap">
          ...
          <div className="flex items-center gap-3">
            ...
            <button
              ...
              aria-label={t("orders.addProduct")}
            ...
            <button
              ...
              className="w-7 h-7 rounded-full border border-hairline grid place-items-center text-secondary hover:text-accent hover:border-accent/50"
              aria-label={t("orders.pickProductFromPhotos")}
              title={t("orders.pickFromPhotos")}
            >
              <ImageIcon size={14} />
            </button>
          </div>
        ...
        {lines.length === 0 ? (
          <p className="text-caption text-secondary py-2">{t("orders.noItemsYet")}</p>
        ) : (
            ...
            <table className="w-full text-subhead min-w-[480px]">
              <thead>
                <tr className="text-caption text-secondary uppercase text-left border-b border-hairline">
                  <th className="px-2.5 py-2 font-medium w-8">{t("orders.columnNumber")}</th>
                  <th className="px-2.5 py-2 font-medium">{t("orders.headerArticle")}</th>
                  <th className="px-2.5 py-2 font-medium">{t("orders.headerDescription")}</th>
                  <th className="px-2.5 py-2 font-medium text-right tabular-nums">{t("orders.headerPrice")}</th>
                  <th className="px-2.5 py-2 font-medium text-center">{t("orders.headerQty")}</th>
                  <th className="px-2.5 py-2 font-medium text-right tabular-nums">{t("common.total")}</th>
                  <th className="w-8" />
                </tr>
              </thead>
              <tbody>
        ...
        >
          ...
          <span className="flex-1 text-subhead text-secondary">{t("orders.searchArticle")}</span>
          <span
            onClick={(e) => {
              e.stopPropagation();
        ...
        {lines.length > 0 && (
          <div className="mt-3 rounded-card bg-canvas p-3.5 space-y-1.5">
            ...
            {gp !== null && (
              <div className="flex items-center justify-between text-subhead">
                <span className="font-semibold">{t("orders.gp")}</span>
        ...
        <div className="mt-4">
          ...
          <textarea
            ...
            placeholder={
              isManager
                ? t("orders.notePlaceholderFor", { recipient: noteRecipient })
                : t("orders.notePlaceholderManager")
            }
            value={noteText}
            ...
            >
              <div className="p-5">
                <div className="flex gap-2.5 items-start">
                  ...
                  <div>
                    <div className="text-headline font-bold">{t("orders.discardOrderTitle")}</div>
                    <div className="text-subhead text-secondary mt-0.5">
                      {lines.length === 1
                        ? t("orders.discardOrderBodyOne", { n: lines.length })
                        : t("orders.discardOrderBodyMany", { n: lines.length })}
                    </div>
                  </div>
                </div>
              </div>
        ...
        >
          <div className="p-5">
            <div className="flex gap-2.5 items-start mb-4">
              ...
              <div>
                <div className="text-headline font-bold">{t("orders.paymentCollection")}</div>
                <div className="text-subhead text-secondary mt-0.5">
                  {t("orders.overdueCustomerPrefix")}{" "}
                  <strong className="text-primary">{t("orders.daysOverdueCount", { days })}</strong>{" "}
                  {t("orders.overdueCustomerSuffix")}
                </div>
              </div>
            </div>
            {showReasonInput ? (
              <>
                <Label>{t("orders.noteForManager")}</Label>
                <TextInput
                  placeholder={t("orders.whyNotCollected")}
                  value={reason ?? ""}
          ...
          <div className="flex gap-2 justify-end px-5 py-3.5 border-t border-hairline">
            {showReasonInput ? (
              <>
                <Button tier="plain" onClick={() => setShowReasonInput(false)}>{t("common.back")}</Button>
                <Button tier="primary" disabled={!reason?.trim()} onClick={onNoteSaved}>{t("orders.saveNote")}</Button>
              </>
            ) : (
              <>
                <Button tier="plain" onClick={onCancel}>{t("common.cancel")}</Button>
                <Button tier="tinted" onClick={() => setShowReasonInput(true)}>{t("orders.giveANote")}</Button>
                <Button tier="primary" onClick={onCollect}>{t("orders.continueToPayment")}</Button>
              </>
            )}
          </div>
        </motion.div>
...
function ProductPhotoPicker({ onClose, onSelect }: { onClose: () => void; onSelect: (p: Product) => void }) {
  ...
  return (
    <Sheet open onClose={onClose} title={t("orders.selectProduct")}>
      ...
      {loading ? (
        <p className="text-caption text-secondary">{t("common.loading")}</p>
      ) : products.length === 0 ? (
        <p className="text-caption text-secondary">{t("orders.noProductsFound")}</p>
      ) : (
