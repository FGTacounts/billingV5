...
export async function POST(req: NextRequest) {
  ...
  if (!photo || photo.size === 0) {
    return NextResponse.json({ error: t("products.photoRequired") }, { status: 400 });
  }
  if (!sku) {
    return NextResponse.json({ error: t("products.skuRequired") }, { status: 400 });
  }
  ...
  try {
    ...
    return NextResponse.json(
      { error: e instanceof Error ? e.message : t("products.driveUploadFailed") },
      { status: 502 }
    );
  }
}
