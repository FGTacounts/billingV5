...
// Cheque status values are stored data; only the label shown in the picker
// is user-facing text.
const CHEQUE_STATUS_LABEL: Record<string, string> = {
  pending: t("payments.statusPending"),
  cleared: t("payments.statusCleared"),
  bounced: t("payments.statusBounced"),
  returned: t("payments.statusReturned"),
};
  ...
  return (
    <div className="p-4 md:p-6 max-w-[1600px] mx-auto">
      <div className="flex items-center justify-between mb-5 gap-3 flex-wrap">
        <h1 className="text-large-title font-bold">{t("nav.payments")}</h1>
        <div className="flex items-center gap-4">
          ...
          <Button tier="primary" onClick={() => setShowLog(true)} className="flex items-center gap-1.5">
            <Plus size={16} /> {t("payments.logPayment")}
          </Button>
        </div>
      </div>
      ...
      {!loading && (
        <div className="grid grid-cols-3 gap-3 mb-5">
          <Card className="p-4 text-center">
            <div className="text-caption text-secondary">{t("payments.collected")}</div>
            <div className="text-title font-bold tabular-nums text-accent">{formatAed(summary.received)}</div>
          </Card>
          <Card className="p-4 text-center">
            <div className="text-caption text-secondary">{t("payments.remaining")}</div>
            <div className="text-title font-bold tabular-nums">{formatAed(summary.remaining)}</div>
          </Card>
          <Card className="p-4 text-center">
            <div className="text-caption text-secondary">{t("payments.overdue")}</div>
      ...
      {loading ? (
        ...
        <EmptyState icon={Banknote} title={t("payments.noPaymentsYet")} />
      ) : (
              ...
              >
                <div className="min-w-0">
                  <div className="text-subhead font-semibold flex items-center gap-1.5">
                    {p.customer?.name ?? t("common.notSet")}
                    {wasEdited(p) && <Pill tone="warning">{t("payments.edited")}</Pill>}
                  </div>
                  <div className="text-caption text-secondary">
                    {p.cheque_number
                      ? `${t("payments.chequeNumbered", { number: p.cheque_number })}${p.cheque_bank ? ` · ${p.cheque_bank}` : ""}`
                      : t("payments.cash")}
                    {" · "}
                ...
                <div className="flex items-center gap-2 shrink-0" onClick={(e) => e.stopPropagation()}>
                  {p.cheque_photo_ref && (
                    <a href={p.cheque_photo_ref} target="_blank" rel="noreferrer" aria-label={t("payments.viewChequePhoto")}>
                      <ImageIcon size={16} className="text-secondary" />
                    </a>
                  )}
                  <span className="text-subhead font-semibold tabular-nums text-end">
                    {formatAed(p.amount)}
                    {(p.discount_amount ?? 0) > 0 && (
                      <span className="block text-caption font-normal text-secondary">
                        {t("payments.plusDiscount", { amount: formatAed(p.discount_amount ?? 0) })}
                      </span>
                    )}
                  </span>
                  {p.cheque_status && (
                    ...
                    >
                      {["pending", "cleared", "bounced", "returned"].map((s) => (
                        <option key={s} value={s}>{CHEQUE_STATUS_LABEL[s]}</option>
                      ))}
                    </select>
                  )}
                  {p.status === "pending" ? (
                    isManager ? (
                      <Button tier="tinted" onClick={() => confirm(p.id)}>{t("payments.confirm")}</Button>
                    ) : (
                      <Pill tone="warning">{t("payments.pending")}</Pill>
                    )
                  ) : (
                    <Pill tone="accent">{t("payments.confirmed")}</Pill>
                  )}
                </div>
              </div>
            );
            ...
            if (!isManager) {
              ...
              for (const m of managers ?? []) {
                await notify(
                  ...
                  m.id,
                  "payment_edited",
                  t("payments.paymentEditedTitle"),
                  t("payments.paymentEditedBody", {
                    user: user.full_name,
                    customer: editing.customer?.name ?? t("payments.aCustomer"),
                  })
                );
              }
            }
