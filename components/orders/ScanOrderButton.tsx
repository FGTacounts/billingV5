...
export default function ScanOrderButton({
  ...
  async function handleFile(file: File) {
    ...
    setOpen(true);
    try {
      // Rule 8: the AI read goes on the queue. /api/jobs writes one row and
      // returns; /api/jobs/run makes the call and records how it went on that
      // row — so a scan that fails leaves a reason behind instead of only a
      // sheet that said "the scan failed". The photo rides with the worker
      // request rather than being written into the row: an 8MB image as
      // base64 in jsonb is a row nobody wants.
      const job = await runQueuedJob("scan.order", { fileName: file.name }, { file });

      if (job) {
        const read = job.result as {
          lines?: ScannedOrderLine[];
          customer?: string | null;
        } | null;
        setLines(read?.lines ?? []);
        setCustomer(read?.customer ?? null);
        tapSuccess();
      } else {
        // No jobs table on this database yet (RUN-ME-23 not run) — scan in the
        // request, exactly as before, rather than losing the camera.
        const fd = new FormData();
        fd.append("file", file);
        const res = await fetch("/api/scan-order", { method: "POST", body: fd });
        const data = await res.json();
        if (!res.ok) {
          setError(data.error ?? t("orders.scanFailed"));
        } else {
          setLines(data.lines ?? []);
      ...
      // friendlyError keeps a real reason ("Scanning isn't set up…") and turns
      // a genuine network failure into the sentence this used to show.
      setError(friendlyError(e, t("common.scanServerUnreachable")));
    }
    setBusy(false);
  }
  ...
  function use() {
    ...
    toast.success(
      matched.length === 1
        ? t("orders.scanAddedOne", { n: matched.length })
        : t("orders.scanAddedMany", { n: matched.length })
    );
    close();
  }
  ...
  return (
    <>
      <button
        ...
        aria-label={t("orders.scanWrittenOrder")}
        title={t("orders.scanWrittenOrder")}
        type="button"
      >
        <Camera size={14} />
      </button>
        ...
        <Sheet
          ...
          footer={
            <>
              ...
              >
                {t("orders.takeAnother")}
              </Button>
              <Button tier="primary" disabled={busy || matched.length === 0} onClick={use}>
                {matched.length === 1
                  ? t("orders.addLineOne", { n: matched.length })
                  : t("orders.addLineMany", { n: matched.length })}
              </Button>
            </>
          }
        >
          {preview && (
            ...
            <img
              src={preview}
              alt={t("orders.scannedDocumentAlt")}
          ...
          {customer && (
            <p className="text-subhead mb-3">
              <span className="text-secondary">{t("orders.customerReadAs")}</span>{" "}
              <span className="font-semibold">{customer}</span>
              <span className="text-caption text-secondary">{t("orders.pickRealCustomer")}</span>
            </p>
          )}
              ...
              <table className="w-full text-subhead">
                <thead>
                  <tr className="text-caption text-secondary uppercase text-left border-b border-hairline">
                    <th className="px-3 py-2 font-medium">{t("orders.headerItem")}</th>
                    <th className="px-3 py-2 font-medium text-center">{t("orders.headerQty")}</th>
                    <th className="px-3 py-2 font-medium text-right tabular-nums">{t("orders.headerPrice")}</th>
                    <th className="w-8" />
                  </tr>
                </thead>
                <tbody>
                    ...
                    >
                      <td className="px-3 py-2.5">
                        ...
                        {l.matched ? (
                          ...
                          <div className="text-caption text-[--status-warning] flex items-center gap-1">
                            <AlertTriangle size={11} /> {t("orders.noProductMatches", { scannedAs: l.scannedAs })}
                          </div>
                        )}
                      </td>
                      ...
                      <td className="px-3 py-2.5">
                        <button
                          ...
                          aria-label={t("orders.removeThisLine")}
                        >
                          <Trash2 size={14} />
                        </button>
                      </td>
