...
export async function approveCustomerChange(
  ...
  if (request.customer_id) {
    ...
    if (!data || data.length === 0) {
      return { ok: false, error: t("customers.updateCustomerFailed") };
    }
  } else {
    const { data, error } = await supabase.from("customers").insert(request.payload).select("id");
    if (error) return { ok: false, error: error.message };
    if (!data || data.length === 0) {
      return { ok: false, error: t("customers.createCustomerFailed") };
    }
  }
...
export async function rejectCustomerChange(
  ...
  if (error) return { ok: false, error: error.message };
  if (!data || data.length === 0) {
    return { ok: false, error: t("customers.reviewPermissionDenied") };
  }
  return { ok: true };
}
