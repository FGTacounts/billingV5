...
export async function POST(req: NextRequest) {
  ...
  if (!username || !name || !password || !role) {
    return NextResponse.json({ error: t("settings.userFieldsRequired") }, { status: 400 });
  }
  if (!["salesman", "manager", "warehouse", "admin"].includes(role)) {
    return NextResponse.json({ error: t("settings.invalidRole") }, { status: 400 });
  }
  ...
  if (authErr || !authUser.user) {
    return NextResponse.json({ error: authErr?.message ?? t("settings.failedToCreateAccount") }, { status: 400 });
  }
