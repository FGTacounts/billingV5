...
export function SalesmanDrilldown({
  ...
  const rangeLabelText =
    SALE_RANGE_PRESETS.find((p) => p.days === days)?.label ?? t("sales.nDays", { n: days });
    ...
    <Sheet
      ...
      headerExtra={
        <select
          ...
          aria-label={t("sales.dateRange")}
        >
          {SALE_RANGE_PRESETS.map((p) => (
            <option key={p.days} value={p.days}>
              {p.label === "Month to date" || p.label === "Year to date" ? p.label : t("sales.lastPeriod", { label: p.label })}
            </option>
          ))}
        </select>
      }
    >
      {loading ? (
        ...
        <div className="grid lg:grid-cols-[1fr_320px] gap-4 items-start">
          <Card className="p-4">
            <div className="text-title font-bold mb-3">{t("sales.sale")}</div>
            <TrendLineChart
            ...
            <div className="mt-2 flex items-center gap-4 text-caption text-secondary">
              ...
              <span className="flex items-center gap-1.5">
                <span className="w-3 h-0.5 rounded-full bg-secondary/50" /> {t("sales.previousPeriod")}
              </span>
            </div>
          </Card>
          ...
          <div className="flex flex-col gap-3">
            ...
            <Card className="p-4 flex items-end justify-between gap-3">
              <div>
                <div className="text-title font-bold leading-tight">{t("sales.sale")}</div>
            ...
            <Card className="p-4 grid grid-cols-3 gap-2">
              <MiniStat label={t("sales.pctToGoal")} value={`${Math.round(pct)}%`} />
              <MiniStat
                label={monthsInRange > 1 ? t("sales.goalPeriod") : t("sales.goal")}
                value={formatCompact(periodTarget)}
                align="center"
              />
              <MiniStat
                label={t("sales.amountLeftLabel")}
            ...
            <Card className="p-4 flex items-center justify-between gap-3">
              <span className="text-title font-bold">{t("sales.avgGpPct")}</span>
              <span className="text-title font-bold tabular-nums text-accent">
                {gpPct === null ? t("common.notSet") : `${Math.round(gpPct)}%`}
              </span>
            </Card>

            <Card className="p-4">
              <div className="flex items-center justify-between gap-2 mb-2">
                <span className="text-title font-bold">{t("nav.orders")}</span>
                <span className="text-subhead font-semibold tabular-nums text-secondary">
                  {orders.length}
                </span>
              </div>
              {orders.length === 0 ? (
                <EmptyStateInline text={t("sales.noOrdersInRange", { range: rangeLabelText.toLowerCase() })} />
              ) : (
                    ...
                    >
                      <div className="min-w-0">
                        <div className="text-subhead font-semibold truncate">
                          {o.customer?.name ?? o.new_customer_note ?? t("sales.unnamedCustomer")}
                        </div>
                      ...
                      <div className="text-right shrink-0">
                        ...
                        <div className="text-[10px] text-secondary tabular-nums">
                          {t("sales.nItems", { n: itemCounts[o.id] ?? 0 })}
                        </div>
                      </div>
                    </div>
                  ))}
