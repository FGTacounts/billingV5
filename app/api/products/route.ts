...
export async function GET(req: NextRequest) {
  ...
  try {
    const products = await fetchProductsServer(supabaseAdmin(), {
      ...
      stockGroupId: req.nextUrl.searchParams.get("stockGroup") ?? undefined,
    });
    return NextResponse.json({ products });
  } catch (e) {
    return NextResponse.json({ error: e instanceof Error ? e.message : t("products.loadFailed") }, { status: 400 });
  }
}
