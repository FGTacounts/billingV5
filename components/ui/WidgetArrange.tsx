  ...
  return (
    <div className="relative">
      <button
        ...
        aria-label={t("ui.arrangeWidgets")}
        title={t("ui.arrange")}
      >
      ...
      {open && (
        <>
          ...
          <div className="absolute end-0 mt-2 w-72 glass rounded-card shadow-floating z-20 p-2">
            <div className="text-caption text-secondary px-2 py-1.5">{t("ui.widgetOrder")}</div>
            {order.map((key, i) => (
                ...
                <div className="flex items-center gap-1 shrink-0">
                  {onToggleHidden && (
                    <button
                      ...
                      aria-label={
                        hiddenSet.has(key)
                          ? t("ui.showWidget", { label: labels[key] })
                          : t("ui.hideWidget", { label: labels[key] })
                      }
                      title={hiddenSet.has(key) ? t("ui.show") : t("ui.hide")}
                    >
                  ...
                  <button
                    ...
                    aria-label={t("ui.moveWidgetUp", { label: labels[key] })}
                  >
                    <ChevronUp size={14} />
                  </button>
                  <button
                    ...
                    onClick={() => onChange(moveKey(order, key, 1))}
                    className="p-1 rounded-inner text-secondary hover:text-accent disabled:opacity-30"
                    aria-label={t("ui.moveWidgetDown", { label: labels[key] })}
                  >
                    <ChevronDown size={14} />
                  </button>
                </div>
                  ...
                  <div className="flex items-center gap-1 shrink-0">
                    {(["full", "half", "quarter"] as const).map((sz) => (
                      <button
                        ...
                        title={t("ui.widgetSizeTitle", { label: labels[key], size: sz })}
                      >
                        {sz[0]}
                      </button>
                    ))}
