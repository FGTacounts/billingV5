...
export async function POST(req: NextRequest) {
  ...
  if (!message) return NextResponse.json({ error: t("inbox.writeMessageFirst") }, { status: 400 });
  ...
  if (!data || data.length === 0) {
    return NextResponse.json({ error: t("inbox.didntPost") }, { status: 403 });
  }
