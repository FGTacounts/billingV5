...
const CONDITION_LABEL: Record<Condition, string> = {
  excellent: t("customers.conditionExcellent"),
  moderate: t("customers.conditionModerate"),
  bad: t("customers.conditionBad"),
};
...
const QUICK_FILTERS: { value: QuickFilter; label: string }[] = [
  ...
  { value: "needsPayment", label: t("customers.paymentsNeeded") },
  { value: "excellent", label: t("customers.conditionExcellent") },
];
...
const SORT_OPTIONS: { key: SortKey; label: string }[] = [
  { key: "name_asc", label: t("customers.sortNameAsc") },
  { key: "name_desc", label: t("customers.sortNameDesc") },
  { key: "balance_desc", label: t("customers.sortBalanceDesc") },
  { key: "balance_asc", label: t("customers.sortBalanceAsc") },
  { key: "sale_desc", label: t("customers.sortSaleDesc") },
  { key: "sale_asc", label: t("customers.sortSaleAsc") },
];
...
const CUSTOMER_COLUMN_LABELS: Record<CustomerColumnKey, string> = {
  district: t("customers.columnDistrict"),
  sale: t("customers.columnSale"),
  payment: t("customers.columnPayment"),
  vat: t("customers.columnVatNumber"),
};
const CUSTOMER_VIEWS: { key: string; label: string; columns: CustomerColumnKey[] }[] = [
  { key: "default", label: t("customers.viewDefault"), columns: ["district", "sale", "payment"] },
  { key: "financial", label: t("customers.viewFinancial"), columns: ["sale", "payment", "vat"] },
  { key: "compact", label: t("customers.viewCompact"), columns: [] },
  { key: "all", label: t("customers.viewAllColumns"), columns: ALL_CUSTOMER_COLUMNS },
];
  ...
  return (
    <div className="relative">
      <button
        ...
        aria-label={t("customers.adjustView")}
        title={t("customers.adjustView")}
      >
      ...
      {open && (
        <>
          ...
          <div className="absolute end-0 mt-2 w-64 glass rounded-card shadow-floating z-20 p-3">
            <div className="text-caption text-secondary font-semibold mb-1.5">{t("customers.defaultViews")}</div>
            ...
            <div className="text-caption text-secondary font-semibold mb-1.5">{t("customers.columns")}</div>
  ...
  return (
    <div className="p-4 md:p-6 max-w-[1600px] mx-auto">
      <div className="flex items-center justify-between mb-5 gap-3 flex-wrap">
        <h1 className="text-large-title font-bold">{t("nav.customers")}</h1>
      ...
      {loading ? (
        ...
        <EmptyState icon={Users} title={t("customers.noneFound")} />
      ) : (
        <div className="bg-surface border border-hairline rounded-card overflow-hidden">
          ...
          <div className="hidden sm:flex items-center justify-between gap-3 px-4 py-2 text-caption text-secondary uppercase font-medium border-b border-hairline">
            <span className="flex-1">{t("customers.columnName")}</span>
            <span
              className={`grid ${
            ...
            >
              {activeCustomerColumns.includes("sale") && <span>{t("customers.columnSale")}</span>}
              {activeCustomerColumns.includes("payment") && <span>{t("customers.columnPayment")}</span>}
              <span>{t("customers.columnBalance")}</span>
            </span>
            <span className="shrink-0 w-[150px] text-right">{t("customers.columnCondition")}</span>
          </div>
          ...
          {pager.visible.map((g) => {
            ...
            const money = (v: number) => (g.hasData ? formatAed(v) : t("common.notSet"));
            if (activeCustomerColumns.includes("sale")) financialCols.push({ key: "sale", label: t("customers.columnSale"), value: money(g.totalSale) });
            if (activeCustomerColumns.includes("payment")) financialCols.push({ key: "payment", label: t("customers.columnPayment"), value: money(paymentColumnValue) });
            financialCols.push({ key: "balance", label: t("customers.columnBalance"), value: money(g.totalDue), emphasis: true });
              ...
              >
                <div className="min-w-0 flex-1">
                  ...
                  <div className="text-caption text-secondary truncate">
                    ...
                    {showVat ? t("customers.vatInline", { vat: g.members[0].vat_number ?? "" }) : ""}
                  </div>
                </div>
                ...
                <div className="flex items-center gap-2 shrink-0 self-end sm:self-auto sm:w-[150px] sm:justify-end">
                  {g.overdue && <Pill tone="danger">{t("customers.overdue")}</Pill>}
                  <StatusSquare
                    tone={g.condition ? CONDITION_TONE[g.condition] : "neutral"}
                    label={g.condition ? CONDITION_LABEL[g.condition] : t("customers.noData")}
                  />
                  {g.members.length === 1 && showPdfQuickDownload && (
                    <a
                      ...
                      aria-label={t("customers.downloadStatementPdf")}
                  ...
                  {g.members.length === 1 && showExcelQuickDownload && (
                    <a
                      ...
                      onClick={(e) => e.stopPropagation()}
                      className="p-1.5 text-secondary hover:text-accent"
                      aria-label={t("customers.downloadStatementExcel")}
      ...
      <Pagination {...pager} noun={t("customers.paginationNoun")} />
...
function CustomerEditor({
  ...
  useEffect(() => {
    ...
    if (!customer) {
      nextCustomerCode(supabase).then((code) => setForm((f) => ({ ...f, code: String(code) })));
    }
  ...
  async function save() {
    ...
    try {
      if (asRequest) {
        ...
        if (!result.ok) {
          toast.error(result.error ?? t("customers.sendForApprovalFailed"));
          return;
        }
        toast.success(
          customer
            ? t("customers.changeSentForApproval")
            : t("customers.newCustomerSentForApproval")
        );
        onSaved();
        return;
      }
  ...
  return (
    ...
    >
      <Label>{t("customers.customerCode")} {!customer && t("customers.customerCodeAutoEditable")}</Label>
      <TextInput
        value={form.code}
      ...
      <Label>{t("customers.shopGroup")}</Label>
      <input
        list="shop-groups"
        ...
        placeholder={t("customers.shopGroupPlaceholder")}
      />
      ...
      {countries.length > 0 && (
        <>
          ...
          >
            <option value="">{t("customers.defaultZone")}</option>
            {countries.map((c) => (
      ...
      <Label>{t("customers.overdueThresholdDays")}</Label>
      <TextInput
        type="number"
