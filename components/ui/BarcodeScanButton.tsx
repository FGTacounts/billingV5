...
export default function BarcodeScanButton({ onScan }: { onScan: (value: string) => void }) {
  ...
  async function start() {
    if (!supported) {
      toast.error(t("ui.barcodeUnsupported"));
      return;
    }
    setError(null);
    setOpen(true);
    ...
    try {
      ...
      setError(t("ui.cameraUnavailable"));
    }
  }
        ...
        <div className="fixed inset-0 z-50 bg-black/80 flex items-center justify-center p-6">
          <div className="bg-surface rounded-card overflow-hidden max-w-sm w-full">
            <div className="flex items-center justify-between px-4 py-3 border-b border-hairline">
              <span className="text-subhead font-semibold">{t("ui.scanBarcode")}</span>
              <button onClick={stop} aria-label={t("common.close")}>
                <X size={18} />
              </button>
            </div>
