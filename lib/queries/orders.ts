...
// The Edited stamp arrives with RUN-ME-19-order-edited-stamp.sql. Until it
// has been run the columns are asked for once, refused, and never asked for
// again — an un-migrated database simply never shows the Edited pill rather
// than failing every order query.
let editedColumnsMissing = false;

// Runs a query that asks for the stamp, and quietly runs it again without it
// where the columns are not there yet. The refusal is matched on the column
// name rather than on the code alone: another missing column (deleted_at on
...
async function withEditedStamp<R extends { error: { code?: string; message?: string } | null }>(
  ...
  if (!editedColumnsMissing) {
    ...
    if (!first.error) return first;
    if (!/edited_(at|by)/.test(first.error.message ?? "")) return first;
    editedColumnsMissing = true;
  }
  return build(ORDER_SELECT);
}
...
async function trashAction(path: string, orderId: string) {
  ...
  if (!res.ok) throw new Error(data.error ?? t("orders.thatDidntWork"));
  return data as {
    ok: true;
...
async function orderRef(supabase: SupabaseClient, orderId: string): Promise<OrderRef> {
  ...
  return {
    ...
    invoice: data?.invoice_number ? `#${data.invoice_number}` : t("orders.theOrder"),
    customerName,
  };
}
...
export async function sendDraft(supabase: SupabaseClient, orderId: string, actorId: string) {
  ...
  await notifyManagers(
    ...
    actorId,
    "order_pending",
    () => t("orders.notifNewOrderToReview"),
    orderId,
...
export async function acceptOrder(supabase: SupabaseClient, orderId: string, actorId: string) {
  ...
  await notifySalesman(
    ...
    actorId,
    "order_accepted",
    (r) => t("orders.notifOrderAccepted", { invoice: r.invoice }),
    () => t("orders.notifOrderAcceptedBody")
  );
}
...
export async function rejectOrder(supabase: SupabaseClient, orderId: string, actorId: string) {
  ...
  await notifySalesman(
    ...
    actorId,
    "order_rejected",
    (r) => t("orders.notifOrderRejected", { invoice: r.invoice }),
    () => t("orders.notifOrderRejectedBody")
  );
}
...
export async function resubmitOrder(supabase: SupabaseClient, orderId: string, actorId: string) {
  ...
  await notifyManagers(
    ...
    actorId,
    "order_pending",
    (r) => t("orders.notifOrderResubmitted", { invoice: r.invoice }),
    orderId,
    (r) => r.customerName ?? ""
  );
}
...
export async function startPicking(supabase: SupabaseClient, orderId: string, actorId: string) {
  ...
  await notifySalesman(
    ...
    actorId,
    "order_picking",
    (r) => t("orders.notifPickingStarted", { invoice: r.invoice }),
    () => t("orders.notifPickingStartedBody")
  );
}
...
export async function markPacked(
  ...
  await logStatus(supabase, orderId, actorId);
  // Who packed it, tagged onto the note the manager reads — the phone has
  // stamped this on hand-off since the picking screen was built
  // (WarehouseOrderPickView.sendToManager), and the manager approving the
  // order should not have to work it out from the status log.
  if (packerName) {
    const tag = t("orders.packedBy", { name: packerName });
    const { data: current } = await supabase
      .from("orders")
      .select("manager_note")
      .eq("id", orderId)
      .maybeSingle();
    const existing = (current?.manager_note ?? "").trim();
    // Never stack the tag when the note already carries it — a re-pack
    // would otherwise add it again every time.
    if (!existing.toLowerCase().includes(tag.toLowerCase())) {
      await supabase
        .from("orders")
        .update({ manager_note: existing ? `${existing} • ${tag}` : tag })
        .eq("id", orderId);
    }
  }
  ...
  await notifyManagers(
    ...
    actorId,
    "order_packed",
    (r) => t("orders.notifPackedWaitingApproval", { invoice: r.invoice }),
    orderId,
    (r) => r.customerName ?? ""
  );
  await notifySalesman(
    ...
    actorId,
    "order_packed",
    (r) => t("orders.notifOrderPacked", { invoice: r.invoice }),
    () => t("orders.notifOrderPackedBody")
  );
}
...
export async function cancelOrder(supabase: SupabaseClient, orderId: string, actorId: string) {
  ...
  await notifySalesman(
    ...
    actorId,
    "order_cancelled",
    (r) => t("orders.notifOrderCancelled", { invoice: r.invoice })
  );
}
