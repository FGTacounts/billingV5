...
export async function POST(req: NextRequest) {
  ...
  if (!user) return NextResponse.json({ error: t("common.notSignedIn") }, { status: 403 });
  ...
  if (orderErr) {
    ...
    if (orderErr.code === "42703") {
      return NextResponse.json(
        { error: t("orders.deleteNeedsMigration") },
        { status: 501 }
      );
    }
  ...
  if (order.deleted_at) return NextResponse.json({ error: t("orders.alreadyInTrash") }, { status: 409 });

  const isManager = user.role === "manager" || user.role === "admin";
  const ownDraft = order.salesman_id === user.id && ["draft", "pending"].includes(order.status);
  if (!isManager && !ownDraft) {
    return NextResponse.json(
      { error: t("orders.onlyManagerCanDeleteAfterHandoff") },
      { status: 403 }
    );
  }
