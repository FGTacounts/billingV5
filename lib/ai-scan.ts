...
async function callGemini(parts: GeminiPart[], systemPrompt: string): Promise<string> {
  ...
  if (!res.ok) {
    ...
    if (res.status === 429) {
      throw new Error(t("documents.aiRateLimited"));
    }
    if (res.status === 404) {
      throw new Error(t("documents.aiModelUnavailable", { model: MODEL }));
    }
  ...
  if (!text) throw new Error(t("documents.aiReturnedNothing"));
  return text;
}
...
function parseJson<T>(raw: string): T {
  ...
  try {
    ...
    throw new Error(t("documents.aiUnreadableAnswer"));
  }
}
