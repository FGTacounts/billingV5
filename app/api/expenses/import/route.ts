...
export async function POST(req: NextRequest) {
  ...
  if (!Array.isArray(rows) || rows.length === 0) {
    return NextResponse.json({ error: t("expense.noRowsToImport") }, { status: 400 });
  }
  ...
  if (clean.length === 0) {
    return NextResponse.json({ error: t("expense.importRowRequirements") }, { status: 400 });
  }
  ...
  if (unknownNames.size > 0) {
    notes.push(t("expense.unknownStaffNames", { names: [...unknownNames].join(", ") }));
  }
  if (attributionDropped) {
    notes.push(t("expense.attributionNeedsMigration"));
  }
