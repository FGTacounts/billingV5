...
export default function ScanCheque({
  ...
  useEffect(() => {
    ...
    navigator.mediaDevices
      ...
      .catch(() => setError(t("payments.cameraAccessDenied")));
  ...
  async function runOcr(canvas: HTMLCanvasElement) {
    ...
    try {
      ...
      setError(t("payments.chequeReadFailed"));
    } finally {
      setScanning(false);
    }
  }
  ...
  return (
    <div className="fixed inset-0 z-[60] bg-black flex flex-col">
      <div className="flex items-center justify-between p-4 text-white">
        <span className="text-subhead font-medium">{t("payments.scanCheque")}</span>
      ...
      <div className="flex-1 relative flex items-center justify-center overflow-hidden">
        ...
        {scanning && (
          <div className="absolute inset-0 bg-black/60 flex flex-col items-center justify-center text-white gap-2">
            ...
            <span className="text-subhead">{t("payments.readingCheque", { progress })}</span>
          </div>
        )}
      </div>
