  ...
  return (
    <Card className="p-4">
      <div className="flex items-center justify-between mb-1">
        <span className="text-caption text-secondary">{t("sales.target")}</span>
        <span className="text-caption text-secondary">{t("dashboard.leftToTarget")}</span>
      </div>
...
function LeaderboardStrip({
  ...
  return (
    <Card className="p-3 h-full flex flex-col">
      <div className="text-title font-bold px-1 pb-2.5 shrink-0">{t("nav.role.salesman")}</div>
  ...
  return (
    <div className="grid lg:grid-cols-[1fr_260px] gap-4 items-start">
      ...
      <div className="flex flex-col gap-3">
        <Card className="p-4">
          <ExpandedStatRow label={t("dashboard.totalSale")} value={formatAed(totalSale)} />
          <ExpandedStatRow label={t("dashboard.avgSale")} value={formatAed(avgSale)} />
        </Card>
        {totalGp > 0 && (
          <Card className="p-4">
            <ExpandedStatRow label={t("sales.totalGp")} value={formatAed(totalGp)} />
            <ExpandedStatRow label={t("dashboard.gpPct")} value={`${Math.round(gpPct)}%`} />
          </Card>
        )}
        {totalProfit !== 0 && (
          <Card className="p-4">
            <ExpandedStatRow label={t("dashboard.totalProfit")} value={formatAed(totalProfit)} />
          </Card>
        )}
        {leaderboard.length > 0 && (
          <Card className="p-4">
            <div className="text-subhead font-semibold mb-2">{t("dashboard.leadingContributors")}</div>
            {topContributor && (
        ...
        <Button tier="plain" onClick={onNavigateSales} className="self-end flex items-center gap-1">
          {t("nav.sales")} <ArrowRight size={14} />
        </Button>
      </div>
    </div>
  );
...
function PaymentsExpanded({
  ...
  return (
    <div className="grid lg:grid-cols-[1fr_240px] gap-4 items-start">
      <Card className="p-4">
        <div className="text-title font-bold mb-3">{t("nav.payments")}</div>
        <MonthlyBarChart
          height={280}
      ...
      <div className="flex flex-col gap-3">
        <Card className="p-4 flex flex-col gap-2.5">
          <div className="flex items-center justify-between">
            <span className="text-subhead font-semibold text-accent">{t("dashboard.collected")}</span>
            <span className="text-headline font-bold tabular-nums text-accent">{formatAed(summary.received)}</span>
          </div>
          <div className="flex items-center justify-between">
            <span className="text-subhead font-semibold text-secondary">{t("dashboard.remaining")}</span>
            <span className="text-headline font-bold tabular-nums text-secondary">{formatAed(summary.remaining)}</span>
          </div>
          <div className="flex items-center justify-between">
            <span className="text-subhead font-semibold text-[--status-danger]">{t("dashboard.overdue")}</span>
        ...
        <Button tier="plain" onClick={onNavigatePayments} className="self-end flex items-center gap-1">
          {t("nav.payments")} <ArrowRight size={14} />
        </Button>
      </div>
    </div>
  );
...
function ExpenseExpanded({
  ...
  return (
    <Card className="p-4">
      <div className="text-title font-bold mb-4">{t("nav.expense")}</div>
      ...
      <div className="flex justify-end mt-4">
        <Button tier="plain" onClick={onNavigateExpense} className="flex items-center gap-1">
          {t("nav.expense")} <ArrowRight size={14} />
        </Button>
      </div>
    </Card>
  );
...
function GoalTableExpanded({
  ...
  if (entries.length === 0) return <EmptyStateInline text={t("sales.noSalesYetThisMonth")} />;

  return (
    <Card className="p-4 overflow-x-auto">
      <div className="grid grid-cols-5 gap-3 mb-4 min-w-[520px]">
        <HeaderStat label={t("dashboard.totalPct")} value={`${Math.round(teamPct)}%`} />
        <HeaderStat label={t("common.total")} value={formatCompact(teamTotal)} />
        <HeaderStat label={t("sales.pctToGoal")} value={`${Math.max(0, Math.round(100 - teamPct))}%`} />
        <HeaderStat label={t("dashboard.amtToGoal")} value={formatCompact(Math.max(0, teamGoal - teamTotal))} />
        <HeaderStat label={t("sales.goal")} value={formatCompact(teamGoal)} />
      </div>
      <div className="flex justify-end min-w-[520px]">
        <button
          ...
          title={ascending ? t("dashboard.sortLowestFirst") : t("dashboard.sortHighestFirst")}
          aria-label={t("dashboard.toggleSortOrder")}
        >
          <ArrowDownUp size={13} />
        </button>
      </div>
      <table className="w-full text-subhead min-w-[520px]">
        <thead>
          <tr className="text-caption text-secondary uppercase text-left">
            <th className="pb-2 font-medium">{t("nav.role.salesman")}</th>
            <th className="pb-2 font-medium text-right">{t("dashboard.salePct")}</th>
            <th className="pb-2 font-medium text-right">{t("sales.sale")}</th>
            <th className="pb-2 font-medium text-right">{t("sales.pctToGoal")}</th>
            <th className="pb-2 font-medium text-right">{t("sales.goal")}</th>
          </tr>
        </thead>
        <tbody>
          {sorted.map((e) => {
...
function CategorySalesWidget({ salesmanId }: { salesmanId?: string }) {
  ...
  return (
    <Card className="p-4">
      <div className="flex items-center justify-between mb-3 gap-2 flex-wrap">
        <span className="text-title font-bold">{t("dashboard.salesByGroup", { group: groupBy })}</span>
        <div className="flex items-center gap-1">
          {(["category", "product"] as const).map((g) => (
            ...
            >
              {g === "category" ? t("dashboard.category") : t("dashboard.product")}
            </button>
          ))}
        </div>
      </div>
      {rows === null ? (
        ...
        <EmptyStateInline text={t("dashboard.noSalesThisMonthYet")} />
      ) : (
        <div className="flex flex-col gap-1.5">
          {rows.map((r) => (
            <div key={r.label} className="relative rounded-inner overflow-hidden bg-canvas">
              <div
                className={`absolute inset-y-0 start-0 ${colorful ? "" : "bg-accent/15"}`}
                style={{
              ...
              <div className="relative flex items-center justify-between gap-3 px-3 py-2">
                ...
                <span className="flex items-center gap-3 shrink-0 tabular-nums">
                  <span className="text-caption text-secondary">{t("dashboard.nUnits", { n: r.units })}</span>
...
const MANAGER_WIDGET_LABELS: Record<(typeof MANAGER_WIDGETS)[number], string> = {
  ...
  pipeline: t("dashboard.widgetPipeline"),
  avgSale: t("dashboard.widgetAvgSale"),
  payments: t("dashboard.widgetPayments"),
  expense: t("dashboard.expenseBreakdown"),
  leaderboard: t("dashboard.salesmanLeaderboard"),
  categorySales: t("dashboard.widgetCategorySales"),
};
...
const SALESMAN_WIDGET_LABELS: Record<(typeof SALESMAN_WIDGETS)[number], string> = {
  stats: t("dashboard.widgetSalesmanStats"),
  newOrder: t("dashboard.widgetNewOrder"),
  pipeline: t("dashboard.widgetSalesmanPipeline"),
  payments3: t("dashboard.widgetPayments3"),
  target: t("dashboard.widgetTarget"),
  trend: t("sales.widgetTrend"),
  monthly: t("dashboard.widgetMonthly"),
};
...
export default function DashboardView({ user }: { user: AppUser }) {
  ...
  const managerWidgets: Record<(typeof MANAGER_WIDGETS)[number], ReactNode> = {
      ...
      <>
        ...
        <div className="hidden lg:grid grid-cols-4 gap-4">
          <StatTile
            ...
            numeric={sales}
            format={formatMoney}
            sub={changePct !== null ? undefined : t("dashboard.vsLastMonthNa")}
            onClick={() => router.push("/sales")}
          />
          <StatTile label={t("sales.grossProfit")} value={formatMoney(gp)} numeric={gp} format={formatMoney} onClick={() => router.push("/sales")} />
          <StatTile
    ...
    pipeline: (
      <StatusPipeline
        onClick={() => router.push("/orders")}
        rows={[
          { label: t("dashboard.pending"), value: String(pending), tone: "warning" },
          { label: t("dashboard.waiting"), value: String(waiting), tone: "info" },
          { label: t("dashboard.packed"), value: String(packed), tone: "accent" },
        ]}
      />
    ),
    avgSale: (
      ...
      >
        <Card className="p-4 h-full flex flex-col">
          <div className="flex items-center justify-between mb-3 gap-2 flex-wrap shrink-0">
            <span className="text-title font-bold">{t("dashboard.averageSale")}</span>
    ...
    payments: (
      ...
      >
        <Card className="p-4 h-full flex flex-col">
          <div className="text-title font-bold mb-3 shrink-0">{t("nav.payments")}</div>
          <MonthlyBarChart
            height={172}
            bars={paymentsByMonth.map((m) => ({
              label: m.label,
          ...
          <div className="grid grid-cols-3 gap-2 mt-3 pt-3 border-t border-hairline">
            <div className="min-w-0">
              <div className="text-caption text-secondary truncate">{t("dashboard.collectedAed")}</div>
            ...
            <div className="min-w-0">
              <div className="text-caption text-secondary truncate">{t("dashboard.remainingAed")}</div>
            ...
            <div className="min-w-0">
              <div className="text-caption text-secondary truncate">{t("dashboard.overdueAed")}</div>
    ...
    expense: (
      ...
      >
        <Card className="p-4 h-full flex flex-col">
          <div className="text-title font-bold mb-3 shrink-0">{t("nav.expense")}</div>
    ...
    leaderboard: (
      <ExpandableWidget
        title={t("dashboard.salesmanLeaderboard")}
  ...
  const salesmanWidgets: Record<(typeof SALESMAN_WIDGETS)[number], ReactNode> = {
    ...
    newOrder: (
      ...
      >
        <Plus size={18} /> {t("dashboard.newOrder")}
      </Button>
    ),
    pipeline: (
      <StatusPipeline
        onClick={() => router.push("/orders")}
        rows={[
          { label: t("dashboard.draft"), value: String(drafts), tone: "neutral" },
          { label: t("dashboard.waiting"), value: String(waiting), tone: "info" },
          { label: t("dashboard.approved"), value: String(approved), tone: "accent" },
        ]}
      />
    ),
    payments3: (
      <div className="flex flex-col gap-2">
        <StatusPipeline
          ...
          rows={[
            { label: t("dashboard.pending"), value: String(paySummary.pendingCount), tone: "warning" },
            { label: t("dashboard.received"), value: formatAed(paySummary.received), tone: "accent" },
            { label: t("dashboard.overdue"), value: formatAed(paySummary.overdue), tone: paySummary.overdue > 0 ? "danger" : "neutral" },
          ]}
        />
        {rejected > 0 && <StatTile label={t("dashboard.rejected")} value={String(rejected)} sub={t("dashboard.rejectedSub")} tone="danger" />}
      </div>
    ),
    target: <TargetBar sale={sales} target={myTarget} />,
    trend: (
      <ExpandableWidget
        title={t("dashboard.saleTrend")}
        expanded={
          <SaleExpanded
            trend={trend}
            prevTrend={prevTrend}
      ...
      >
        <Card className="p-4">
          <div className="flex items-center justify-between mb-3 gap-2 flex-wrap">
            <span className="text-title font-bold">{t("dashboard.saleTrend")}</span>
            <DateRangePicker range={saleRange} onChange={changeSaleRange} />
          </div>
    ...
    monthly: (
      <ExpandableWidget
        title={t("dashboard.paymentsAndYearComparison")}
        expanded={
          <div className="flex flex-col gap-4">
            ...
            />
            <Card className="p-4">
              <div className="text-title font-bold mb-3">{t("dashboard.saleYearComparison")}</div>
    ...
    <div className="p-4 md:p-6 max-w-[1600px] mx-auto">
      <div className="flex items-center justify-between mb-5 gap-3">
        ...
        <div className="min-w-0">
          <h1 className="text-large-title font-bold truncate">
            {t("dashboard.helloName", { name: user.full_name?.split(" ")[0] || user.username })}
          </h1>
      ...
      {isWarehouse && resume && (
        <Card className="p-4 mb-4">
          <div className="text-caption text-secondary mb-1">{t("dashboard.whereYouLeftOff")}</div>
          <div className="flex items-center justify-between gap-3 flex-wrap">
            <div>
              <div className="text-title font-bold">
                {resume.customerName ?? t("dashboard.order")}
                {resume.invoiceNumber ? t("dashboard.invoiceSuffix", { number: resume.invoiceNumber }) : ""}
              </div>
      ...
      {isWarehouse && (
        <StatusPipeline
          rows={[
            { label: t("dashboard.ordersToPick"), value: String(toPick), tone: "warning" },
            { label: t("dashboard.packedAwaitingApproval"), value: String(packedAwaiting), tone: "accent" },
          ]}
        />
      )}
