...
function normalizeHeader(raw: string): string {
  return String(raw ?? "")
    .trim()
    // The sample sheet marks a required column with a trailing star
    // ("Code*"). Left on, the filled-in sample came back with a column called
    // "code*", matched nothing, and was refused for missing its required
    // fields — the one file that should always import.
    .replace(/\s*\*+$/, "")
    .toLowerCase()
    .replace(/[\s-]+/g, "_");
}
