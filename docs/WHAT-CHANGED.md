...
# Round: the pre-ship check (5 Sep 2026)

Everything below was measured against your live Supabase, signed in as a
real manager, a real salesman and a real warehouse account in turn.

## What was checked, and what it said

| Check | Result |
|---|---|
| `products_safe` / `order_items_safe` (the views RUN-ME-13 broke) | **Fixed.** 200 for all three roles |
| Cost prices | Manager and admin see them; salesman and warehouse get null, on products *and* order lines |
| Every query the app makes, run for real, per role | **41 of 41 pass**, manager, salesman and warehouse alike |
| Every page, fetched signed-in, per role | All render; role gates hold (salesman has no Reports or Expense, warehouse has no Sales, Reports, Expense or Planning, and gets Picking) |
| Production build | Clean. No errors, no warnings |
| TypeScript across the whole project | Clean |
| iOS build | Succeeds |
| Invoice numbers | 535 orders, 535 numbers, **no duplicates**; the gapless numbering function is installed and refuses a bad order id without burning a number |
| VAT arithmetic | `subtotal + vat = total` on all 535 orders. No nulls |
| Payments split | Remaining **488,976.50** (≤90 days), Overdue **739,035.62** (>90 days) — exactly the rule you set |

## Speed

Every screen now paints from data in well under a second:

| Screen | Time to first paint |
|---|---|
| Dashboard | 409 ms |
| Sales | 162 ms |
| Orders (first 50) | 187 ms |
| Payments (first 50) | 172 ms |
| Customers (the list) | 121 ms |

The two genuinely heavy figures — the Collected/Remaining/Overdue tile and
the customer ledger walk — arrive a moment later on their own, so nothing
waits on them.

## Two bugs found and fixed

**"Orders this month" said 0 next to AED 7,799.07 of sales.** The count was
made by asking how many orders were *first written* this month, across every
status. The money beside it was made by adding up *delivered* orders by their
*billing* date. Six orders were billed in September and none were drafted in
it, so the two numbers had nothing to do with each other. They are now
literally the same rows counted and added, on the Dashboard, on Sales, and on
the Orders header. The count also became free, since those rows are already
in hand.

**An order could be created already delivered.** `/api/orders/create` runs
with full database rights and only declared in a type annotation that a new
order is a draft or pending — nothing enforced it when the request actually
arrived. A caller posting `status: "delivered"` got an order that had skipped
picking, packing and delivery, held no invoice number, and had moved no
stock: a sale in every total and nowhere in the pipeline. Now refused.

**The iPhone home-screen widget disagreed with the app.** It had its own idea
of what counts — every order that wasn't a draft, rejection or cancellation,
dated by when it was written, valued with VAT included — so it always showed
a bigger number than the app it sat next to. It now follows the same rule as
the dashboard. Its draft counter, which had been reading from a list that by
definition contains no drafts and so had always shown zero, was fixed too.

---

# 18 September 2026 — payments, returns, routes, imports, statement, discounts

**One thing for you to run:** `scratchpad/RUN-ME-25-payment-discounts-and-grv-requests.sql`
in Supabase → SQL Editor. Both apps work without it, but until it is run a
return typed while collecting cannot be raised as a request (the app tells
you so), the discount figure is not stored as its own column (it still comes
off the invoices), and a manager cannot edit or remove a return's lines.

Still waiting from before, and the reason `npm run test:tenancy` fails today:
`scratchpad/RUN-ME-24-safe-views-take-two.sql`.

## 1. Collecting a payment

| Before | Now |
|---|---|
| A discount was a sentence in the payment's notes. No invoice came down. On the phone the payment was recorded *smaller* by the discount, so the customer ended up owing more. | Cash **and** discount come off the invoices you ticked, oldest first, then the customer's other invoices. |
| A goods return typed while collecting was also just a sentence. No manager ever saw it. | It is raised as a **GRV request** for that amount, already carrying the customer. It is in Payments → Returns (GRV) and the manager's Inbox. |
| — | The manager opens the request, enters the products that came back, and taps **Save & approve**. Only then does the amount come off the customer's balance. A request can't be approved with no products on it. |
| Editing a payment's amount changed nothing anyone is aged against. | Saving an edit re-applies the amount and discount to the same invoices. Managers and admins can also change the discount and put a confirmed payment back to pending. Admins could not edit payments on the phone at all; they can now. |
| A return could never be corrected. | Managers and admins can change a return's amount, note and products — even after approval (stock moves by the difference) — and remove a pending request. |
| The GRV report listed returns by customer only. | It also lists **Returned products** (approved returns), with a CSV. |

## 2. Route planning

The phone **did** have the page — More → Planning — but it stayed blank until
a city was typed, so it looked missing. It now opens on a recommended route.
On both apps the route puts **payment collection first**: overdue customers,
then anyone who owes anything, then the idle-customer visits — worked out
from what customers really owe after payments, discounts and returns. You can
drag stops, remove one for the day, and the arrangement is remembered on that
device until tomorrow; **Back to recommended** undoes it. A manager picks a
salesman to see that salesman's route.

## 3. Import sample sheets

Every sample now lists every column its import reads, with an example in each
cell; only the starred columns are required. Three real faults were fixed on
the way:

- Re-importing a sheet that left a column out **wiped that column** on
  existing products and customers (a stock sheet reset every price to 0). A
  blank now means "keep what is there".
- The downloaded sample itself would not upload, because of the `*` in its
  headers.
- The customers sample starred District, Address and VAT No as required; they
  never were.

New optional columns: customers — Country, Salesman, Active; products —
Active; orders — PO Number, Note.

## 4. Statement PDF

Same letterhead, title bar, boxes, table and brand strip as the invoice, and
it now runs to as many pages as the account needs. The old one silently
stopped at the bottom of page one while still printing the full closing
balance.

## 5. Per-product discount on an order

Managers and admins get a **Disc %** column on an order's lines (while the
order is a draft or pending, the same window in which prices can be edited).
Typing 10 charges the list price less 10%; the invoice's Discount column
shows it. Salesmen and the warehouse do not see the column, and the server
refuses the request from anyone else.

## 6. The customer's old price — tested

`npm run test:money` now proves the rule with 12 cases (the old price beats
the list price whether the list price has gone up or down, beats the standing
discount, loses to a price written on a scanned or imported document, and a
remembered price of zero is ignored). One fault found and fixed: adding an
article to an order already written ignored the customer's standing discount.

`npm run test:prices` checks the live book, read-only. **What it found
matters:** there are 530 billed orders and not one of them has line items, so
no customer has an old price yet. The history was loaded as invoice totals
only. Old prices will build up from orders approved in the app from now on;
to have them for past invoices, the old invoice *lines* would need importing.

## 7. Second round the same day

**Statements.** The statement you download is now only what the customer still
owes. Beside the **Paid orders** row there is a **Paid statement** button for
the settled invoices. The customer's name is the biggest thing in the Bill To
box, is repeated on later pages, and is in the file name and the Excel tab —
"Statement M&SAVE SUPERMARKET LLC (20417) 18.09.2026".

**Bank.** FAB, exactly as on your sheet. **Please check one thing:** your sheet
says ACCOUNT NO …433**000** but the IBAN on the same sheet contains
…433**001**. The IBAN is valid as written. I printed both as you gave them —
if the account line is a typo, tell me the right number.

**Photos.** Every tile was costing the server five separate network trips, one
after another; the thumbnail change on 12 September added the fifth, which is
why it got slower. The server now keeps thumbnails it has already fetched, so
a photo anyone has seen recently comes back without going to Google at all.
The first person to open a category after a restart still waits for Google
once per photo; everyone after that does not. This helps the phone too — it
uses the same route. I could not time it from here (the app is behind your
login), so please tell me how it feels.

**On the phone.** The real cause of slow photos there was different and
bigger: the app never asked for a small picture. Every tile downloaded the
full-size original and shrank it on the handset. Tiles now ask for a thumbnail
sized to the tile, only six load at once with whatever is on screen going
first, tiles that scroll away stop loading, and a photo already shown in the
grid is not fetched again when you open the article. The phone's customer
sheet also gained the **Paid orders** row (it did not have one) with the
**Paid statement** button on it.

## 8. Salesman-wise statement

Reports → **Salesman Statement**. Pick a salesman and you get every invoice
from orders they took that is not yet paid in full — customer, total,
received, balance, days, and which are overdue — with **PDF** and **Excel**
downloads in the same design as the customer statement. The balances are the
same ones the customer's own statement shows, so the two always agree. On the
...
- Nothing here has been clicked through while signed in: the app is behind a
  login I do not have. Everything was verified by type-check, production
  build, the arithmetic tests, rendered sample PDFs, and by confirming every
  changed route refuses a stranger.
- `npm run test:tenancy` **fails**, on `order_items_safe` only — the same
