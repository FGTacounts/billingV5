...
// The stored value is the role itself, so what the button says is looked up
// rather than being the value — the catalogue owns the words.
...
export default function LoginForm() {
  ...
  async function submit() {
    ...
    if (signInError) {
      setError(t("account.unknownUsernameOrPassword"));
      setLoading(false);
      return;
    }
    ...
    if (profile && actual !== "admin" && actual !== role.toLowerCase()) {
      ...
      setError(
        t("account.registeredAs", {
          role: `${actual.charAt(0).toUpperCase()}${actual.slice(1)}`,
        })
      );
      setLoading(false);
      return;
  ...
  return (
    <div className="relative min-h-[100dvh] grid place-items-center bg-gradient-to-br from-[#5ccaa6] to-[#2f8c70] p-6 overflow-hidden">
      ...
      <div
        aria-hidden
        className="pointer-events-none absolute -top-1/3 -start-1/4 w-[80vw] h-[80vw] max-w-[720px] max-h-[720px] rounded-full opacity-40 blur-3xl"
        style={{ background: "radial-gradient(circle, rgba(255,255,255,0.55), transparent 65%)" }}
      />
      <div
        aria-hidden
        className="pointer-events-none absolute -bottom-1/3 -end-1/4 w-[70vw] h-[70vw] max-w-[620px] max-h-[620px] rounded-full opacity-30 blur-3xl"
      ...
      >
        ...
        <p className="text-white/85 mt-0.5 mb-7 tracking-wide">{t("account.billingSystem")}</p>
